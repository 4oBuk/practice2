package com.chornobuk;

/**
 * Hello world!
 *
 */
public class App 
{
    public static void main( String[] args )
    {
        System.out.println( "Hello World!" );
    }
    // todo:
    // [x]1. create class for fine
    // [x]2. create class for statistic
    // [x]3. write parser to read one rule's break in XML (can rewrite method from taks 1)
    // [x]4. test it
    // [x]5. write general method that will parse one file and return map.entry
    //      name-data using method from step 3
    // [x]6. method for reading all files from folder and retur map of results
    // [x]7. create method for generating statistic object 
    // [x] test it
    // [x]8. create method for writing statistic object in file in JSON format
    // []9. create main method that will use above methods to read dat from files
    // create statistic and write result in JSON file. This method will take path to the folder
    // and result file path
    // []10. test method
    // []11. refactoring
}
